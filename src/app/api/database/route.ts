  ...
  try {
    ...
    if (action === 'testConnection') {
      ...
      return NextResponse.json({ 
        success: true, 
        data: { message: 'MySQL database is connected' }
      });
    }
    ...
    if (action === 'list' && model) {
      ...
      switch (model) {
        ...
        case 'categories':
          ...
          break;
        case 'promos':
          data = await prisma.promo.findMany();
          break;
        default:
          return NextResponse.json(
      ...
      case "create":
        ...
        switch (model) {
          ...
          case 'category':
            ...
            break;
          case 'promo':
            result = await prisma.promo.create({ data });
            break;
          default:
            return NextResponse.json(
              { success: false, error: "Invalid model" },
      ...
      case "update":
        ...
        switch (model) {
          ...
          case 'booking':
            ...
            break;
          case 'category':
            updatedResult = await prisma.category.update({
              where: { id },
              data: updateData
            });
            break;
          case 'promo':
            updatedResult = await prisma.promo.update({
              where: { id },
              data: updateData
            });
            break;
    ...
    if (action === "delete" && model && id) {
      ...
      switch (model) {
        ...
        case 'category':
          ...
          break;
        case 'promo':
          result = await prisma.promo.delete({ where: { id } });
          break;
        default:
          return NextResponse.json(
            { success: false, error: "Invalid model" },
