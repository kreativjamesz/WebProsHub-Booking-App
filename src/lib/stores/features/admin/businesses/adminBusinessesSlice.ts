...
export const fetchBusinesses = createAsyncThunk(
  ...
  async ({ page = 1, search = '', status = 'all', category = 'all', city = 'all' }: {
    page?: number;
    search?: string;
    status?: string;
    category?: string;
    ...
    const response = await fetch(`/api/admin/businesses?page=${page}&search=${search}&status=${status}&category=${category}&city=${city}`, {
      headers: {
...
export const bulkUpdateBusinesses = createAsyncThunk(
  ...
  async ({ businessIds, updates }: { businessIds: string[]; updates: Partial<Business> }) => {
    // TODO: Replace with actual API call
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(updateBusinessDetails.rejected, (state, action) => {
        state.isUpdatingBusiness = false;
        state.error = action.error.message || 'Failed to update business details';
      });
    ...
    builder
      ...
      .addCase(bulkUpdateBusinesses.fulfilled, (state, action) => {
        ...
        action.payload.businesses.forEach((updatedBusiness: Business) => {
          const index = state.businesses.findIndex(business => business.id === updatedBusiness.id);
          if (index !== -1) {
