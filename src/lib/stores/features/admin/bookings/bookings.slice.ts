...
import { BookingsState } from "./bookings.types";
...
export const fetchBookings = createAsyncThunk(
  ...
  async ({
    ...
    const response = await fetch(
      `/api/admin/bookings?page=${page}&search=${search}&status=${status}&date=${date}&business=${business}&customer=${customer}`
    );
...
export const getBookingsAnalytics = createAsyncThunk(
  ...
  async ({ startDate, endDate }: { startDate: string; endDate: string }) => {
    // TODO: Replace with actual API call
    const response = await fetch(
      `/api/admin/bookings/analytics?startDate=${startDate}&endDate=${endDate}`
    );
    const data = await response.json();
    return data;
  }
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(updateBookingStatus.fulfilled, (state, action) => {
        ...
        const index = state.bookings.findIndex(
          (booking: Booking) => booking.id === action.payload.id
        );
        if (index !== -1) {
    ...
    builder
      ...
      .addCase(updateBookingDetails.rejected, (state, action) => {
        state.isUpdatingBooking = false;
        state.error =
          action.error.message || "Failed to update booking details";
      });
    ...
    builder
      ...
      .addCase(bulkUpdateBookings.fulfilled, (state, action) => {
        ...
        action.payload.bookings.forEach((updatedBooking: Booking) => {
          const index = state.bookings.findIndex(
            (booking: Booking) => booking.id === updatedBooking.id
          );
          if (index !== -1) {
